...
public class FoodService {
	...
	private Food getFood(JSONObject json) throws JSONException
	{
		...
		ingredients.put("polyunsaturated_fat",sanityCheck(food, "polyunsaturated_fat"));
		...
		ingredients.put("calories_from_fat",sanityCheck(food, "calories_from_fat"));
		
		Log.i(LOG_TAG, "cal_from_fat");
		
		ingredients.put("monounsaturated_fat",sanityCheck(food, "monounsaturated_fat"));
		...
		for (int i = 0; i < servings.length(); i++) {
			...
			if (serving == serving_type.getInt("serving_type")) 
			{
				...
				Food newFood = new Food(food.getString("food_description"), serving_type.getString("name"));
			 ...
			 Iterator<String> itr = ingredients.keySet().iterator();
			 		while(itr.hasNext())
			 		{
			 			...
			 			if(len > 3)
			 			newFood.SetIngredient(key,finalVal.substring(0,3));
			 			}
			 			}
			 		}
			 return newFood;
